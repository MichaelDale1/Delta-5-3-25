...
output "instance_external_ips" {
  value = {
    ...
    vm2 = "http://${google_compute_instance.sao-paulo1-vm.network_interface[0].access_config[0].nat_ip}"
    vm3 = "http://${google_compute_instance.tokyo1-vm.network_interface[0].access_config[0].nat_ip}"
  }
...
output "lb_static_ip_address-iowa" {
  description    = "The static IP address of the Iowa load balancer."
  value  = "http://${google_compute_address.lb.address}"
 }


# https://developer.hashicorp.com/terraform/language/functions/join
output "compute_zones-iowa" {
